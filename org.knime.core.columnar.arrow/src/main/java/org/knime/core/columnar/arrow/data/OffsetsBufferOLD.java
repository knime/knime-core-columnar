...
final class OffsetsBufferOLD {
    ...
    public static int usedSizeFor(final int capacity) {
        return (capacity + 1) * Integer.BYTES; // offsets
     ...
     * @param capacity the number of elements
     */
    public OffsetsBufferOLD(final int capacity) {
        m_offsets = new int[capacity + 1];
        m_nextIndex = 0;
    }

    /**
     * Constructs an {@code OffsetsBuffer} from an existing offsets array. Use {@link #createFrom(ArrowBuf, int)}.
     *
     * @param offsets the offsets array
     */
    private OffsetsBufferOLD(final int[] offsets) {
    ...
    /**
     * Returns the last offset value. This is the end offset of the last element and therefore the length of the
     * flattened data.
     *
    ...
    public int getStartIndex(final int index) {
        if (index < 0 || index >= m_offsets.length - 1) {
            throw new IndexOutOfBoundsException(
                "Index " + index + " out of bounds for valid range 0 to " + (m_offsets.length - 2));
        }
     ...
     */
    public static OffsetsBufferOLD createFrom(final ArrowBuf offsetBuffer, final int length) {
        int[] offsets = new int[length + 1];
        MemoryCopyUtils.copy(offsetBuffer, offsets);
        return new OffsetsBufferOLD(offsets);
    }
}
