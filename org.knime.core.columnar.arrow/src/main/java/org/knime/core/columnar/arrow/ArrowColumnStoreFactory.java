...
public class ArrowColumnStoreFactory implements ColumnStoreFactory {
    ...
    public ArrowBatchStore createStore(final ColumnarSchema schema, final FileHandle fileSupplier) {
        return new OnHeapArrowBatchStore(schema, fileSupplier, m_compression, newChildAllocator("ArrowColumnStore"));
    }

    @Override
    @SuppressWarnings("resource") // Allocator closed by store
    public ArrowBatchReadStore createReadStore(final Path path) {
        return new OnHeapArrowBatchReadStore(path, newChildAllocator("ArrowColumnReadStore"));
    }
     ...
     */
    @SuppressWarnings("resource") // Allocator closed by store
    public SequentialBatchReadable createPartialFileReadable(final Path path,
        final OffsetProvider offsetProvider) {
        return new OnHeapArrowPartialFileBatchReadable(path, offsetProvider,
