 ...
 */
public final class OffHeapArrowVoidData implements VoidWriteData, OffHeapArrowWriteData, VoidReadData, OffHeapArrowReadData {
    ...
    private OffHeapArrowVoidData(final NullVector vector, final int capacity) {
        m_vector = vector;
        m_capacity = capacity;
        m_length = vector.getValueCount();
    }

    private OffHeapArrowVoidData(final NullVector vector, final int capacity, final int length) {
        m_vector = vector;
        m_capacity = capacity;
        m_length = length;
    }
    ...
    @Override
    public OffHeapArrowVoidData close(final int length) {
        if (length < 0) {
        ...
        return new OffHeapArrowVoidData(m_vector, m_capacity);
    }
    ...
    @Override
    public OffHeapArrowVoidData slice(final int start, final int length) {
        return new OffHeapArrowVoidData(m_vector, m_capacity, length);
    }

    /** Implementation of {@link OffHeapArrowColumnDataFactory} for {@link OffHeapArrowVoidData} */
    public static final class ArrowVoidDataFactory implements OffHeapArrowColumnDataFactory {
        ...
        @Override
        public OffHeapArrowVoidData createRead(final FieldVector vector, final ArrowVectorNullCount nullCount,
            final DictionaryProvider provider, final ArrowColumnDataFactoryVersion version) throws IOException {
            if (CURRENT_VERSION.equals(version)) {
                return new OffHeapArrowVoidData((NullVector)vector, vector.getValueCapacity());
            } else {
