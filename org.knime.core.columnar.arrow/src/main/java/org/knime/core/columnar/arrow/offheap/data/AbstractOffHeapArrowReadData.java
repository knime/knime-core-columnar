...
/**
 * Abstract implementation of {@link OffHeapArrowReadData}. Holds a {@link FieldVector} of type F in {@link #m_vector} and an
...
abstract class AbstractOffHeapArrowReadData<F extends FieldVector> extends AbstractReferencedData implements OffHeapArrowReadData {
    ...
    /**
     * Create an abstract {@link OffHeapArrowReadData} with the given vector, an offset of 0 and the length of the vector.
     *
     * @param vector the vector
     * @param missingValues if all, no or some values are missing
     */
    public AbstractOffHeapArrowReadData(final F vector, final MissingValues missingValues) {
        this(vector, missingValues, 0, vector.getValueCount());
    }

    /**
     * Create an abstract {@link OffHeapArrowReadData} with the given vector.
     *
     *
     * @param vector the vector
     * @param missingValues if all, no or some values are missing
     ...
     */
    public AbstractOffHeapArrowReadData(final F vector, final MissingValues missingValues, final int offset,
        final int length) {
        m_vector = vector;
        m_offset = offset;
        m_length = length;
    ...
    protected void closeResources() {
        if (m_offset != 0) {
            throw new IllegalStateException("Trying to close a AbstractOffHeapArrowReadData with m_offset != 0");
        }
        m_vector.close();
    }
