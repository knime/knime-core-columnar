 ...
 */
public final class OffHeapMemoryAlertSystem {

    private static final Logger LOGGER = LoggerFactory.getLogger(OffHeapMemoryAlertSystem.class);

    /** The singleton instance of the {@link OffHeapMemoryAlertSystem} */
    public static final OffHeapMemoryAlertSystem INSTANCE = new OffHeapMemoryAlertSystem();

    private final Set<OffHeapMemoryAlertListener> m_listeners;

    private final AtomicBoolean m_lastReleaseResult;

    private final Semaphore m_semaphore;

    private OffHeapMemoryAlertSystem() {
        m_listeners = ConcurrentHashMap.newKeySet();
        m_semaphore = new Semaphore(1);
        m_lastReleaseResult = new AtomicBoolean(false);
    }
    ...
    public boolean sendMemoryAlert() throws InterruptedException {
        // TODO should there be a non-blocking version?
    ...
    /** Interface for listeners on the {@link OffHeapMemoryAlertSystem}. */
    public interface OffHeapMemoryAlertListener {

        /**
         * This method is called if the off-heap memory is low. Listeners should release release resources. If new
         * off-heap allocations would exceed the memory limit they are blocked until this method returns.
         *
