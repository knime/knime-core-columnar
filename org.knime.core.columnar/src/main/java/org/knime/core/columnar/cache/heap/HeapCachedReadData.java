...
final class HeapCachedReadData<T> implements ObjectReadData<T> {
    ...
    HeapCachedReadData(final HeapCachedWriteData<T> delegate, final Object[] data, final int length) {
        m_writeData = delegate;
        m_data = data;
        m_length = length;
    }
    ...
    public synchronized long sizeOf() {
        return m_readDelegate != null ? m_readDelegate.sizeOf() : m_writeData.sizeOf();
    }
    ...
    synchronized ObjectReadData<T> serialize() {
        m_writeData.serialize(m_length);

        m_readDelegate = m_writeData.getDelegate().close(m_length);
        if (m_numRetainsPriorSerialize > 0) {
