 ...
 */
public final class OnHeapReadBeforeFullyWrittenDictionaryTest {
    ...
    void testReadBeforeFullyWrittenDictionary() throws IOException {
        ...
        final OnHeapArrowColumnDataFactory[] factories =
            new OnHeapArrowColumnDataFactory[]{new DictionaryEncodedDataFactory()};
        ...
        try (final RootAllocator allocator = new RootAllocator()) {
            ...
            final OnHeapArrowBatchWriter writer =
                new OnHeapArrowBatchWriter(writePath, factories, ARROW_NO_COMPRESSION, allocator);
            ReadBatch batch;
            ...
            @SuppressWarnings("resource")
            final SequentialBatchReader reader = new OnHeapArrowPartialFileBatchReader(writePath.asFile(), allocator,
    ...
    private static ReadBatch fillBatchDict(final WriteBatch batch, final int chunkSize, final long seed) {
        ...
        OnHeapTestData.fillData(data, chunkSize, seed);
    ...
    private static void assertBatchDataDict(final ReadBatch batch, final int chunkSize, final long seed) {
        final DictionaryEncodedData data = (DictionaryEncodedData)batch.get(0);
        OnHeapTestData.checkData(data, chunkSize, seed);
    }
