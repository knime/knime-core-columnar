...
public final class TestBatchStore implements BatchStore {
    ...
    public static TestBatchStore create(final ColumnarSchema schema) {
        final TestBatchStore store = new TestBatchStore(schema);
        ColumnarTest.OPEN_CLOSEABLES.add(store);
        ColumnarTest.OPEN_CLOSEABLES.add(store.m_writer);
        return store;
    }

    private TestBatchStore(final ColumnarSchema schema) {
        m_schema = schema;
