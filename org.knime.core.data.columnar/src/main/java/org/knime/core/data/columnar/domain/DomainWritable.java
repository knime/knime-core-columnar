...
public final class DomainWritable implements BatchWritable, Closeable {
    ...
    private final class DomainBatchWriter implements BatchWriter {
        ...
        DomainBatchWriter(final BatchWriter delegate) {
    ...
    private static final NodeLogger LOGGER = NodeLogger.getLogger(DomainWritable.class);

    private static final String ERROR_ON_INTERRUPT = "Interrupted while waiting for domain calculation thread.";

    private final ColumnarSchema m_schema;

    private final DomainBatchWriter m_writer;
    ...
    private DomainWritableConfig m_config;
    ...
    public DomainWritable(final BatchWritable delegate, final DomainWritableConfig config,
        final ExecutorService executor) {
        m_schema = delegate.getSchema();
        m_writer = new DomainBatchWriter(delegate.getWriter());
        m_config = config;
        m_executor = executor;
    }
