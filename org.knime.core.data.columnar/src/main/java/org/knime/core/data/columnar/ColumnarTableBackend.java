...
public final class ColumnarTableBackend implements TableBackend {
    ...
    @Override
    public KnowsRowCountTable[] slice(final ExecutionContext exec, final BufferedDataTable table, final Selection[] slices,
        final IntSupplier tableIdSupplier) {
        try {
            final int numSlices = slices.length; //NOSONAR
            var tableSlices = new KnowsRowCountTable[numSlices];
            for (int i = 0; i < slices.length; i++) {//NOSONAR
