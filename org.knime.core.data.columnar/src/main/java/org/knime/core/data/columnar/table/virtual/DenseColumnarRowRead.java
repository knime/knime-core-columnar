...
/**
 * A {@link RowRead} whose values are backed by a {@link ReadAccessRow}.
 *
...
final class DenseColumnarRowRead implements RowRead {
    ...
    DenseColumnarRowRead(final ValueSchema schema, final ReadAccessRow readAccessRow) {
        ...
        for (var i = 0; i < numColumns; i++) {
            final var access = readAccessRow.getAccess(i);
            m_accesses[i] = access;
            m_values[i] = schema.getValueFactory(i).createReadValue(access);
        }
    ...
    public boolean isMissing(final int index) {
        // TODO consider adding a isMissing method on ReadValue
