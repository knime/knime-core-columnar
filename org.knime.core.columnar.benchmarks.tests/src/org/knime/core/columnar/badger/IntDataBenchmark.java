...
final class IntDataBenchmark {
    ...
    void benchOldImpl() throws IOException {
        // TODO this benchmark should also use the object cache because the heap badger replaces it
        // var objCache = new ObjectCache(m_store, m_store, ColumnarPreferenceUtils.getHeapCache(),
        //     ColumnarPreferenceUtils.getPersistExecutor(), ColumnarPreferenceUtils.getSerializeExecutor());
        try (var cursor = ColumnarWriteCursorFactory.createWriteCursor(m_store)) {
            var rowAccess = cursor.access();
    ...
    void runBenchmarks(final BenchmarkFn benchmarkFn) throws IOException {
        int[] numRowsOptions = new int[]{10, 100};
        int[] numColsOptions = new int[]{10};
        MissingValues[] missingValuesOptions =
            new MissingValues[]{MissingValues.NONE, MissingValues.SOME, MissingValues.ALL_};

        for (var numRows : numRowsOptions) {
            m_numRows = numRows;
            for (var numCols : numColsOptions) {
                m_numCols = numCols;
                for (var missingValues : missingValuesOptions) {
